...
vec3 StarLayer(vec2 uv, float glow, float density, float hueShift, float speed, float twinkle) {
  vec3 col = vec3(0.0);
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      float twk = trisn(uTime * speed + seed * 6.2831) * 0.5 + 1.0;
      twk = mix(1.0, twk, twinkle);
      star *= twk;

      col += star * size * base * density;
    }
  }
  return col;
}
...
void main() {
  ...
  for (float i = 0.0; i < 1.0; i += 1.0 / NUM_LAYER) {
    ...
    col += StarLayer(uv * scale + i * 453.32, uGlowIntensity, uDensity, uHueShift, uSpeed, uTwinkleIntensity) * fade;
  }
...
export default function OglGalaxy({
  focal = [0.5, 0.5],
  rotation = [1.0, 0.0],
  starSpeed = 0.4,
  density = 1.1,
  ...
  useEffect(() => {
    ...
    function resize() {
      renderer.setSize(ctn.offsetWidth, ctn.offsetHeight, false);
      if (program) {
  ...
  return <div ref={ctnDom} className={`ogl-galaxy ${className}`} style={style} />;
}
